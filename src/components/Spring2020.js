...
class Spring2020 extends React.Component {
...
loadVoteCount() {
  ...
  handleEvent = (id,selected,buttonClass,votedId) => {
    ...
    if (selected) {
      ...
      localStorage.setItem('voted/'+submission.uid,localStorage.getItem('votes/'+submission.uid)+1);
  ...
  updateVotes = () => {
    this.state.submissions.map(submission=> {
      ...
      let votedId = 'voted/'+submission.uid;
       ...
       db.ref('/submissions').orderByChild('uid').equalTo(submission.uid).once("value", function(snapshot) {
      snapshot.forEach((child) => {
           db.ref('/submissions/'+ child.key).update({'votes': submission.votes})
      });      
    })
    })    
  };
...
if (this.state.voted) {
  ...
  this.setState({
    buttonClass: 'gong-pressed'  },this.logVotes)
} else {
  this.setState({
...
logVotes() {
  ...
  updateVote() {
    ...
    if (this.state.voted) {
      const db = Firebase.database();
      db.ref('/submissions').orderByChild('uid').equalTo(this.props.id).once("value", function(snapshot) {
        snapshot.forEach((child) => {
...
//     this.props.onVote(this.props.id,this.state.voted);
  ...
  sendVote() {
    ...
    this.props.onVote(this.props.id,this.state.voted,this.state.buttonClass,this.votedId);
